...
class TestSmallTransientBypass:
    """Margin-absorbed transients (est ≤ margin/4) run UNACCOUNTED — no
    reservation, no eviction, no waiting. This is the explicit form of the
    economics the stress suite was validated on: reserving small forwards
    turned each one into an admission event (victim evictions, reloads with
    ~2× load-temporary spikes, synchronized wake-ups) — the churn behind the
    2026-06-09 crash campaign. Only TTS-class transients reserve."""
    ...
    @pytest.mark.asyncio
    async def test_tts_sized_transient_still_reserves(self, monkeypatch):
        pool = _make_pool()
        _patch_cap(monkeypatch, 50 * GiB)  # budget 46, threshold 1 GiB
        _add_entry(pool, "tts", size=4 * GiB, busy=True)
        async with pool.reserve_inference("tts", int(2.4 * GiB)):
            assert pool._inflight_reservations == int(2.4 * GiB)
        assert pool._inflight_reservations == 0
