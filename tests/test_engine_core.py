    ...
    async def test_abort_request_wakes_blocked_stream_outputs(
        ...
        with patch("omlx.engine_core.get_registry") as mock_registry:
            ...
            try:
                ...
                # AttributeError on missing .layers).  This keeps the
                # engine loop idle so the abort path below is what wakes
                # the consumer, which is what we actually want to test.
                # Mirrors test_generate_cancel_multiple_requests below.
    ...
    async def test_generate_cancel_aborts_request(self, mock_model, mock_tokenizer):
        ...
        with patch("omlx.engine_core.get_registry") as mock_registry:
            ...
            try:
                ...
                # cancellation path below is what drives the abort.
                # Mirrors test_generate_cancel_multiple_requests below.
                engine.scheduler.has_requests = lambda: False
