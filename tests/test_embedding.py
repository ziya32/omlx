...
class TestEmbeddingEngine:
    ...
    def test_engine_clears_metal_cache_after_embed(self):
        ...
        with patch("omlx.engine.embedding.MLXEmbeddingModel") as MockModel, \
             patch("omlx.engine.base.locked_sync_and_clear_cache") as mock_clear:
            mock_model = MagicMock()
            mock_model.embed.return_value = EmbeddingOutput(
                embeddings=[[0.1, 0.2]],
                total_tokens=5,
            ...
            # sync + clear are bundled in the locked helper, dispatched on the
            # executor by base._finish_activity.
    ...
    def test_engine_clears_metal_cache_per_concurrent_request(self):
        ...
        every request clears, not just the last one. The clear is bundled in
        ``locked_sync_and_clear_cache`` (dispatched on the executor by
        ``base._finish_activity``), so we patch and count that.
        """
        engine = EmbeddingEngine("test-model")
        concurrency = 4
        ...
        with patch("omlx.engine.embedding.MLXEmbeddingModel") as MockModel, \
            ...
            assert mock_clear.call_count == concurrency
