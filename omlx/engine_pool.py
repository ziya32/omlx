...
class EngineEntry:
    ...
    model_type: Literal["llm", "vlm", "embedding", "reranker", "llm_reranker", "asr", "tts"]  # Model type
    engine_type: Literal["batched", "simple", "embedding", "reranker", "llm_reranker", "vlm", "asr", "tts"]  # Engine type to use
    estimated_size: int  # Pre-calculated from safetensors (bytes)
    config_model_type: str = ""  # Raw model_type from config.json (e.g., "deepseekocr_2")
    engine: BaseEngine | EmbeddingEngine | RerankerEngine | LLMRerankerEngine | ASREngine | TTSEngine | None = None  # Loaded engine instance
...
class EnginePool:
    ...
    async def _load_engine(self, model_id: str) -> None:
        ...
        try:
            ...
            elif entry.engine_type == "llm_reranker":
                engine = LLMRerankerEngine(
                    model_name=entry.model_path,
                    scheduler_config=self._scheduler_config,
                )
            elif entry.engine_type == "asr":
                engine = ASREngine(model_name=entry.model_path)
            elif entry.engine_type == "tts":
