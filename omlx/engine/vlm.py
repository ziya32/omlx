...
def _patch_torch_free_image_processor():
    ...
    processors. This patch wraps the affected mlx-vlm processors'
    ``from_pretrained`` so they substitute the PIL class when
    ``AutoImageProcessor`` raises.
    """
...
class VLMBatchedEngine(BaseEngine):
    ...
    async def start(self) -> None:
        ...
        # Apply pre-load patches (native MTP runtime patch for Qwen3.5/3.6
        # + Gemma4 + DeepSeek-V4, DeepSeek-V4 model registration, etc.)
        # before the model is instantiated, so the patched ``__init__``
        # runs. ``maybe_apply_pre_load_patches`` is a no-op when the model
        # is incompatible. Without this call, mtp_enabled=True on a VLM
        # silently does nothing — the MTP heads never attach.
        try:
        ...
        def _load_vlm_sync():
            ...
            with _strip_audio_config_if_orphaned(Path(self._model_name)), \
                ...
                # global output-scale side-car that MLX's quantized_matmul /
                # gather_qmm can't fold in. Attach via the shared post-load hook
                # (same one the LLM / dflash engines use). No-op for any other
                # checkpoint. apply() already ran in maybe_apply_pre_load_patches.
                from ..utils.model_loading import maybe_attach_global_scales
                maybe_attach_global_scales(model, self._model_name)
