...
class ProcessMemoryEnforcer:
    ...
    def _propagate_memory_limit(self) -> None:
        ...
        for entry in self._engine_pool._entries.values():
            ...
            if scheduler is not None:
                ...
                # Plumb the per-engine model weight size for the predictive
                # generation memory guard's per-request peak estimate.
    ...
    async def _check_and_enforce(self) -> None:
        ...
        Eviction is gated on the absolute ceiling (not on the SOFT/HARD
        watermarks). SOFT/HARD signals fire proactively to throttle
        admission before the ceiling is hit; eviction is the last-resort
        lever once it IS hit. Eviction target is the soft watermark —
        recover with margin so oscillation at the boundary doesn't
        trigger repeated evictions.
        ...
        # = min(static, dynamic, metal_cap)). SOFT/HARD watermarks above
        # only drive admission pausing via ``_propagate_memory_limit`` —
        # they don't trigger eviction, so in-flight work drains naturally
        # before any request abort.
        #
        # Gating on the absolute ceiling (rather than SOFT) matters under
        # the stricter ``max(mx.get_active_memory(), phys_footprint)``
        # measurement (phys_footprint includes IOAccelerator + dirty
        # file-backed, routinely 10-20% above mx.get_active_memory alone),
        # which otherwise triggers eviction-thrash on stress workloads.
        if current <= ceiling:
            return
            ...
            while self._current_usage_bytes() > target:
                ...
                if _busy_only_non_pinned:
                    logger.warning(
                        ...
                        "already signaled.",
                        _format_gb(current), _format_gb(ceiling),
                    )
                    break
