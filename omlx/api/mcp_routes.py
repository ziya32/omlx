...
# Auth is wired by server.py at router-include time via
# ``app.include_router(mcp_router, dependencies=[Depends(verify_api_key)])``.
# Tests that exercise the bare router (not via the production app) get an
# unauthenticated router — they can install their own dependency via
# ``app.dependency_overrides`` or by re-including the router with their own
# ``dependencies=`` list.
